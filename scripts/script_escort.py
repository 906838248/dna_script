...
class ScriptEscort(BaseAutomationThread):
    def run(self):  
        for i in range(self.loop_count):
            ...
            self.load_recording("护送/escort_1")
            self.play_recording()
            ...
            if self.find_image("map1.png", timeout=3):
                ...
                self.load_recording("护送/map1_1")
                self.play_recording()
                self.recorder.wait_for_playback(timeout=300)
                self.load_recording("护送/map1_2")
                self.play_recording()
                self.recorder.wait_for_playback(timeout=300)
                self.load_recording("护送/map1_3")
                self.play_recording()
                self.recorder.wait_for_playback(timeout=300)

            elif self.find_image("map2.png", timeout=3):
                self.log_signal.emit("确认为地图类型二")
                self.load_recording("护送/map2_1")
                self.play_recording()
                self.recorder.wait_for_playback(timeout=300)
                self.load_recording("护送/map2_2")
                self.play_recording()
                self.recorder.wait_for_playback(timeout=300)
                self.load_recording("护送/map2_3")
                self.play_recording()
                self.recorder.wait_for_playback(timeout=300)
            elif self.find_image("map3.png", timeout=3):
                self.log_signal.emit("确认为地图类型三")
                self.load_recording("护送/map3_1")
                self.play_recording()
                self.recorder.wait_for_playback(timeout=300)
                self.load_recording("护送/map3_2")
                self.play_recording()
                self.recorder.wait_for_playback(timeout=300)
                self.load_recording("护送/map3_3")
                self.play_recording()
                self.recorder.wait_for_playback(timeout=300)

            elif self.find_image("map4.png", timeout=3):
                self.log_signal.emit("确认为地图类型四")
                self.load_recording("护送/map4_1")
                self.play_recording()
                self.recorder.wait_for_playback(timeout=300)
                self.load_recording("护送/map4_2")
                self.play_recording()
                self.recorder.wait_for_playback(timeout=300)
                self.load_recording("护送/map4_3")
                self.play_recording()
                self.recorder.wait_for_playback(timeout=300)
