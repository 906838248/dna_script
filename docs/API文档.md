...
**说明**: 
- 使用模板匹配算法查找图片
- **支持多分辨率**：自动检测屏幕分辨率并缩放模板图片
- **特征点匹配**：当模板匹配失败时，使用ORB特征点进行分辨率无关的匹配
- 支持模板图片缓存，避免重复加载
- 支持屏幕截图缓存，减少频繁截图操作

**多分辨率支持**:
- 基准分辨率：1920x1080
- 自动缩放：根据当前分辨率自动调整模板大小
- 智能匹配：优先使用快速模板匹配，失败时自动切换到特征点匹配
- 无需为不同分辨率准备多套图片
...
1. **分辨率**: 建议使用1920x1080分辨率截图（基准分辨率）
   - 系统会自动适配其他分辨率（如2560x1440、1366x768等）
   - 无需为不同分辨率准备多套图片
   - 特征点匹配确保跨分辨率识别准确性
2. **格式**: PNG格式，支持透明度
...
5. **多分辨率优化**: 
   - 系统自动检测屏幕分辨率并调整匹配策略
   - 优先使用快速的模板匹配，失败时自动切换到特征点匹配
   - 特征点匹配虽然较慢但能保证跨分辨率识别准确性

## 多分辨率支持详解

### 工作原理

1. **分辨率检测**: 启动时自动检测当前屏幕分辨率
2. **缩放计算**: 计算相对于基准分辨率(1920x1080)的缩放因子
3. **模板缩放**: 根据缩放因子自动调整模板图片大小
4. **双重匹配策略**:
   - **第一层**: 模板匹配（快速，适合相同或相近分辨率）
   - **第二层**: ORB特征点匹配（较慢，适合不同分辨率）

### 配置参数

在 `src/constants.py` 中可以调整以下参数：
